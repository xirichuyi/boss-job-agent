...
test("navigation timeout checks actual loaded page without a second navigation", async () => {
  ...
  b.call = async (method) => {
    if (method === "Page.navigate") {
      navigations++;
      throw new Error("Page.navigate 超时");
    }
    return {
      result: {
        value: {
  ...
  assert.equal(
    (await b.navigate("https://www.zhipin.com/job_detail/example.html"))
...
test("verification redirect stops instead of reporting success", async () => {
  ...
  b.call = async (method) =>
    method === "Page.navigate"
      ...
      : {
          result: {
            value: {
              url: "https://www.zhipin.com/web/passport/verify",
              hasBody: true,
              ready: "complete",
            },
          },
  ...
  await assert.rejects(
    b.navigate("https://www.zhipin.com/job_detail/example.html"),
    /验证/,
  );
});
