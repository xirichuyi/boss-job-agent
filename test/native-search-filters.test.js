...
import { nativeFiltersForSearch, searchRequestFilters } from '../src/job-filters.js';
...
test('native salary rotation covers every keyword and all configured bands', () => {
  ...
  assert.throws(()=>nativeFiltersForSearch(0,8,{...c,nativeSalaryCodes:[]}));
});
test('request evidence extracts native POST filters without credentials', () => {
  const r=searchRequestFilters({url:'https://www.zhipin.com/wapi/zpgeek/search/joblist.json',postData:'salary=405&jobType=1901&scale=304%2C305%2C306&token=private'});
  assert.equal(r.salary,'405');assert.equal(r.scale,'304,305,306');
  assert.equal(r.jobType,'1901');assert.equal(r.token,undefined);
});
test('native controls use exp DOM key and API experience field', async () => {
  const b=new VisibleTools(), expressions=[];
  b.evaluate=async e=>{expressions.push(e);new Function('return '+e);return true;};
  b.until=async e=>{expressions.push(e);new Function('return '+e);return true;};
...
test('active search workflow passes configured filters before reading jobs', () => {
  const s=fs.readFileSync(new URL('../src/workflows/search.js',import.meta.url),'utf8');
  assert.ok(s.includes('jobs.searchKeyword(report.query, report.nativeFilters, slot.city)'));
