...
import { authorizedMessage, ingest, statusText } from '../src/telegram-chat.js';
const since = '2026-09-08T00:00:00Z';
const message = { update_id: 10, message: { chat: { type: 'private', id: 42 }, from: { id: 42 }, date: Date.parse(since)/1000 + 1, text: '进度' } };
test('chat accepts only bound private sender after activation', () => {
  ...
  assert.equal(authorizedMessage({ ...message, message: { ...message.message, from: { id: 5 } } }, 42, since), false);
  assert.equal(authorizedMessage({ ...message, message: { ...message.message, chat: { type: 'group', id: 42 } } }, 42, since), false);
...
test('queue and offset survive duplicate updates; outsiders never queued', () => {
  ...
  ingest(state, [message, { ...message, update_id: 11, message: { ...message.message, from: { id: 9 } } }], 42);
  ingest(state, [message], 42);
  assert.equal(state.offset, 12); assert.equal(state.queue.length, 1);
