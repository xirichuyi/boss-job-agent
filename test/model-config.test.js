...
import { loadModelConfig, MODEL, REASONING_EFFORT } from '../src/model-config.js';
...
test('configuration supports changes without code edits and fails closed', t => {
  ...
  fs.writeFileSync(file, JSON.stringify({ model: 'test-model', reasoningEffort: 'low' }));
  assert.deepEqual(loadModelConfig(file), { model: 'test-model', reasoningEffort: 'low' });
  for (const value of [{}, { model: 'valid', reasoningEffort: 'invalid' }, null]) {
