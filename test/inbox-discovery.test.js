...
test('verified unread contacts are checked first, unknown contacts never sent to', async t => {
  ...
  await checkInbox({root,ledger,report,progress(){},save(){},assertAuthority(){},decide:()=>assert.fail('no unanswered message'),chat:{
    scanConversations:async()=>({complete:false,rows:[{label:'b',recruiter:'HR',unread:true},{label:'unknown',recruiter:'Other HR',unread:true}]}),
    openConversation:async job=>{checked.push(job.id);return {messages:[{self:true,text:'已答复'}]};},
    sendText:()=>assert.fail('must not send')
  }});
  assert.deepEqual(checked,['b','a']);
  const discovery=JSON.parse(fs.readFileSync(root+'/memory/inbox-discovery.json'));
  assert.equal(discovery.unverified.length,1); assert.equal(discovery.reachedRenderedEnd,false);
});
