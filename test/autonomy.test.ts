...
import { reconcileQuota } from "../src/storage/state.ts";
const cycle = {
  id: "test-cycle",
...
test("multiple skipped decisions still get distinct file names", () => {
  const names = new Set(
    Array.from({ length: 100 }, () => decisionKey(cycle.id, "reply")),
  );
...
test("transient read/model failures recover without sending again", () => {
  ...
  assert.equal(
    recoveryPlan(
      { ...cycle, reason: "worker_timeout_result_unconfirmed" },
      report,
      { contacts: [] },
    ).action,
    "retry",
...
test("unknown send is quarantined and reserved quota cannot be refunded", () => {
  const attempt = {
    ...report,
    intents: [{ jobId: "job1", status: "outcome_unknown" }],
  };
  const ledger = {
    contacts: [
      { job: { id: "job1" }, cycle: cycle.id, status: "outcome_unknown" },
    ],
  };
...
test("unidentifiable send and missing crash records stop for review", () => {
  ...
  assert.equal(
    recoveryPlan(
      cycle,
      { ...report, intents: [{ jobId: "missing", status: "outcome_unknown" }] },
      { contacts: [] },
    ).action,
    "review",
  );
...
test("login recovery resumes only after a health check", () => {
  ...
  assert.equal(
    recoveryPlan(auth, report, { contacts: [] }).action,
    "authentication",
  );
  assert.equal(
    recoveryPlan(auth, report, { contacts: [] }, true).action,
    "retry",
  );
});
test("inbox cursor covers older contacts instead of last-five starvation", () => {
  const contacts = Array.from({ length: 23 }, (_, id) => ({
    job: { id },
    status: "delivered",
  }));
  let cursor = 0;
  const seen = new Set();
  for (let i = 0; i < 3; i++) {
    const b = selectInboxBatch(contacts, cursor);
    b.entries.forEach((e) => seen.add(e.job.id));
    cursor = b.next;
  }
