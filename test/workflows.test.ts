...
function context() {
  return {
    cycle: { newContactAllocation: 3, at: new Date().toISOString() },
    ledger: { contacts: [] },
    report: {
      jobReviews: [],
      intents: [],
...
test("AI greeting is sent verbatim, without a code-generated prefix or suffix", async () => {
  ...
  const message =
    "您好，我对这个方向挺感兴趣，之前做过企业知识库项目，方便聊聊吗？";
  c.list = [job];
  c.jobs.detail = async () => ({
    text: "职位描述需求梳理HR",
    buttons: [{ text: "立即沟通" }],
  ...
  c.chat.openConversation = async () => ({
    messages: [{ text: "平台默认招呼 送达", self: true }],
  });
...
test("duplicate company is audited and never reaches browser writes", async () => {
  const c = context();
  c.list = [{ id: "job", company: "company" }];
  c.ledger.contacts = [{ job: c.list[0] }];
  await contactJobs(c);
  assert.equal(c.report.jobReviews.length, 1);
  assert.match(c.report.jobReviews[0].rejected, /账本防重/);
});
test("disabled contact is skipped without an unknown intent and later cards continue", async () => {
  ...
  c.decide = () => ({
    action: "contact",
    reason: "匹配",
    message: "您好，我对这个岗位有兴趣，之前做过相关开发。",
  });
  const checked = [];
  c.jobs.contactReady = async (job) => {
    checked.push(job.id);
    return false;
  };
...
test("model skip does not send; completed task is not reported as successful outreach", async () => {
  ...
  c.list = [job];
  c.jobs.detail = async () => ({ text: "JD", buttons: [{ text: "立即沟通" }] });
  c.chat.searchHistory = async () => ({ empty: true });
  c.decide = () => ({ action: "skip", reason: "专业不匹配" });
  await contactJobs(c);
  c.report.status = "completed";
  const s = summarizeCycle(c.report);
  assert.equal(c.report.intents.length, 0);
  assert.equal(s.outcome, "completed_no_send");
  assert.equal(s.reasons["模型判断不匹配"], 1);
});
test("detail timeout skips one card; authentication stops the workflow", async () => {
  const c = context();
  c.list = [
    { id: "job", location: "杭州", salary: "11-20K", scaleEvidence: "native" },
  ];
  c.jobs.detail = async () => {
    throw Error("页面内容未就绪");
  };
...
test("production batch path generates once and sends separately after fresh history checks", async () => {
  ...
  c.decide.batch = (items) => {
    calls++;
    assert.deepEqual(
      items.map((x) => x.job.id),
      ["a", "b"],
    );
    return items.map(({ job }) => ({
      action: "contact",
      reason: "匹配",
      message: "我对" + job.id + "有兴趣，之前做过相关项目，方便聊聊吗？",
    }));
  };
  c.jobs.contactOnce = async () => {};
  c.chat.openConversation = async () => ({
    messages: [{ text: "默认招呼 送达", self: true }],
  });
  c.chat.sendText = async (job, text, history, persist) => {
    persist();
    sends.push([job.id, text]);
    return { text: "送达" };
  };
  await contactJobs(c);
  assert.equal(calls, 1);
  assert.deepEqual(reads, ["a", "b", "a", "b"]);
  assert.deepEqual(
    sends.map((x) => x[0]),
    ["a", "b"],
  );
  assert.equal(c.report.result.newContacts, 2);
});
