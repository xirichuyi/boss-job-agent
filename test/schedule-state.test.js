...
import { shanghaiDay, cycleStatus, reconcileQuota } from '../src/schedule-state.js';
import { parseCompanySize } from '../src/ranker.js';
...
test('unfinished stale cycles never appear as successful dispatches', () => {
  assert.equal(cycleStatus({ id: 'a', status: 'queued', at: '2026-09-07T00:00:00Z' }, Date.parse('2026-09-07T00:21:00Z')).state, 'stalled');
  assert.equal(cycleStatus({ id: 'a', status: 'blocked', reason: 'navigation failed' }).reason, 'navigation failed');
});
test('unused successful reservations are released exactly once', () => {
  const c = { id: 'a', at: '2026-09-07T00:00:00Z', status: 'completed', newContactAllocation: 3, result: { newContacts: 1 } };
