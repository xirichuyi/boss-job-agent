...
function setup(t, alerts) {
  ...
  for (const [name, value] of Object.entries({
    "telegram-secrets": { token: "TEST" },
    "telegram-config": { chatId: 1 },
    alerts,
  }))
  ...
  const root = setup(t, [
    {
      ...
      kind: "authentication",
      status: "open",
      telegram: { retryAt: new Date(Date.now() + 60000).toISOString() },
    },
    { id: "due", kind: "authentication", status: "open" },
  ]);
  let body;
  await flushTelegram(root, async (_, options) => {
...
test("ordinary alerts and missing welcome marker send nothing", async (t) => {
  const root = setup(
    t,
    ["hr_pending", "history_read_failed", "review", "outcome_unknown"].map(
      (kind) => ({ kind, status: "open" }),
    ),
  );
  const result = await flushTelegram(root, () =>
    assert.fail("unsolicited routine notification"),
  );
  assert.equal(result.sent, 0);
  assert.equal(
    JSON.parse(fs.readFileSync(root + "/memory/alerts.json")).length,
    4,
  );
});
test("authentication alert is retained and sent once", async (t) => {
  const root = setup(t, [
    { id: "auth1", kind: "authentication", status: "open", reason: "需要扫码" },
  ]);
  let sends = 0;
...
test("exhausted contact recovery sends one summary, ordinary retries remain quiet", async (t) => {
  const root = setup(t, [
    ...
    { id: "pending", kind: "contact_recovery_pending", status: "open" },
  ]);
  let calls = 0;
  const fetcher = async () => {
    calls++;
