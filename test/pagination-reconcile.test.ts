...
} from "../src/adapters/browser/pagination.ts";
import {
  observedReceipt,
  reconcileUnknown,
} from "../src/application/reconcile.ts";
import { nextSearchPage } from "../src/application/search.ts";
...
test("pagination scrolls inner container rather than the outer window", () => {
  ...
  assert.equal(
    vm.runInNewContext(advanceJobsExpression, {
      document,
...
test("pagination uses a unique enabled native next-page control", () => {
  ...
  const document = {
    querySelector: () => ({ parentElement: null }),
    querySelectorAll: () => [button],
  };
  assert.equal(
    vm.runInNewContext(advanceJobsExpression, { document }),
    "next_page",
  );
...
test("next page processes only fresh IDs currently rendered", async () => {
  ...
  const list = await nextSearchPage(
    {
      report,
      progress() {},
      jobs: {
        loadMoreJobs: async () => true,
        listJobs: async () => [{ id: "a" }, { id: "b" }],
      },
    },
    2,
  );
...
test("read-only reconciliation requires a fresh unique exact delivered message", () => {
  ...
  for (const messages of [
    ...
    assert.equal(observedReceipt(pending, { messages }), null);
  assert.equal(
    observedReceipt({ ...pending, beforeIds: [null] }, { messages: [m] }),
    null,
  );
});
test("reconciliation never resends or increases agent totals", async () => {
  const entry = {
    job: { id: "a" },
    status: "quarantined",
    pendingWrite: { kind: "text", message: "你好", beforeIds: ["old"] },
  };
  ...
  await reconcileUnknown({
    ...
    chat: {
      openConversation: async () => ({
        messages: [{ id: "new", self: true, text: "送达\n你好" }],
      }),
      sendText: () => assert.fail("must never write"),
    },
  });
  assert.equal(entry.status, "delivered");
  assert.equal(report.result.messagesSent, 0);
...
test("old unknown attempts without baseline remain isolated", async () => {
  ...
  await reconcileUnknown({
    ...
    report: {},
    save() {},
    chat: { openConversation: () => assert.fail("no evidence baseline") },
  });
