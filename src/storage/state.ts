...
import { harness, stateKey } from "./harness.ts";
...
export function atomicBatch(entries, cycle = null) {
  const store = harness();
  if (!store || entries.some(([file]) => !stateKey(file)))
    throw Error("Harness未迁移或事务包含非核心状态");
  const values = entries.map(([file, data]) => [stateKey(file), data]);
  if (cycle) store.admit(values, cycle);
  else store.batch(values);
...
export function cycleStatus(cycle, now = Date.now()) {
  ...
  if (["completed", "skipped"].includes(cycle.status))
    return { state: cycle.status, cycle: cycle.id };
  if (cycle.status === "blocked")
    return {
      state: "blocked",
      cycle: cycle.id,
      reason: cycle.reason || "cycle_blocked",
    };
  const started = Date.parse(cycle.at);
  if (!Number.isFinite(started) || now - started > 20 * 60_000)
    return {
      state: "stalled",
      cycle: cycle.id,
      reason: "cycle_not_finished_within_20_minutes",
    };
...
export function reconcileQuota(quota, cycle) {
  ...
  return {
    ...quota,
    reserved: Math.max(0, quota.reserved - (reserved - used)),
    settledCycles: [...(quota.settledCycles || []), cycle.id],
  };
}
