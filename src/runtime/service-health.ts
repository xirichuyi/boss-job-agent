...
export function effectiveStatus(status, service) {
  ...
  return {
    ...status,
    state: service.state === "unknown" ? "service_unknown" : "stopped",
    phase: undefined,
    reason: "调度服务当前未运行，以下周期信息只是历史记录",
    previousState: status.state,
  };
}
export function watchdogPlan(
  { enabled, maintenance, service, ageSeconds, bootGrace, attempts },
  now = Date.now(),
) {
  if (!enabled) return "disabled";
  if (Date.parse(maintenance?.until || "") > now) return "maintenance";
  if (service === "unknown" || ["activating", "deactivating"].includes(service))
    return "observe";
  if (service === "active" && (bootGrace || ageSeconds < 1200))
    return "healthy";
