...
export function businessHealth(
  ...
  cycle,
  policy = { failureCycles: 3, minReadFailures: 3 },
) {
  if (
    !cycle ||
    !["completed", "blocked", "skipped", "quarantined"].includes(cycle.status)
  )
    return previous;
  ...
  const reasons = [];
  if (readFailed >= policy.minReadFailures)
    reasons.push("会话历史持续读取失败");
  if ((inbox.retryDeferred || 0) >= policy.minReadFailures)
  ...
  return {
    ...
    state:
      failureStreak >= policy.failureCycles
        ? "degraded"
        : reasons.length
          ? "warning"
          : "healthy",
    ...
    reasons,
    readFailed,
    unverified: s.inboxDiscovery?.unverified || 0,
    action:
      failureStreak >= policy.failureCycles
        ? "检查失败联系人和浏览器状态；不因业务告警强杀服务"
        : "observe",
  };
}
