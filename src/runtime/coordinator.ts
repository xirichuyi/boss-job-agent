...
export function executionConfig() {
  ...
  if (typeof config.parallelWorkflows !== "boolean")
    throw Error("并行调度配置无效");
  const r = config.contactRecovery;
  if (
    !r ||
    !["perRun", "minutes", "maxAttempts", "retryMinutes"].every(
    ...
    throw Error("联系人恢复配置无效");
  config.inboxRetry = {
    baseMinutes: 30,
    maxMinutes: 240,
    escalateAfter: 3,
    readRetryMinRemainingMs: 60000,
    ...config.inboxRetry,
  };
  ...
  if (
    ...
    i.maxMinutes < i.baseMinutes ||
    i.maxMinutes > 1440
  )
    throw Error("收件箱退避配置无效");
  config.businessHealth = {
    failureCycles: 3,
    minReadFailures: 3,
    ...config.businessHealth,
  };
  if (
    !Object.values(config.businessHealth).every(
      (v) => typeof v === "number" && Number.isInteger(v) && v > 0,
    )
  )
...
export class PriorityMutex {
  ...
  active: boolean;
  owner: AsyncLocalStorage<{ active: boolean }>;
  constructor() {
    this.queue = [];
    this.active = false;
    this.owner = new AsyncLocalStorage();
  }
  run(task, priority = 0) {
...
export async function runParallelLanes(
  ...
  const results = await Promise.allSettled(
    Object.entries(lanes).map(async ([name, run]) => {
      try {
        return await run();
      } catch (error) {
        onFailure(name, error);
