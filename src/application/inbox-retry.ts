...
export function stopAll(error) {
  return (
    ...
    classifyFailure(error.message) === "authentication"
  );
}
export function deferInbox(root, entry, error, now = Date.now()) {
  ...
  const minutes = Math.min(
    policy.maxMinutes,
    policy.baseMinutes * 2 ** Math.min(attempts - 1, 20),
  );
  entry.inboxRetry = {
    attempts,
    reason: error.message,
    nextAt: new Date(now + minutes * 60000).toISOString(),
    needsAttention: attempts >= policy.escalateAfter,
  };
  // A write that reached its callback stays unknown. Never reset or resend it here.
  if (attempts >= policy.escalateAfter)
    raiseAlert(root, {
      kind: "inbox_recovery_exhausted",
      contact: entry.job.id,
      reason:
        "某联系人持续处理失败，已延长重试间隔；其他联系人继续，详情见状态记录。",
    });
}
