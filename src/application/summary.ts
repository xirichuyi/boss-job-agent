export function summarizeCycle(report) {
  ...
  for (const job of report.jobReviews || []) {
    const why =
      job.rejected ||
      (job.decision?.action === "skip"
        ? job.decision.retryable
          ? "模型调用或输出异常，待重试"
          : "模型判断不匹配"
        : null);
    if (!why) continue;
    const category = /详情读取失败/.test(why)
      ? "详情读取失败"
      : /已有联系人|账本防重|已联系或按钮/.test(why)
        ? "已有联系或发送状态不明"
        : why;
  ...
  return {
    ...
    detailReads: (report.jobReviews || []).filter((j) => j.text).length,
    reasons,
    outcome:
    ...
    contactRecovery: report.contactRecovery || {},
    contactFailures: report.contactFailures || [],
    modelDeferredUntil: report.modelDeferredUntil || null,
  };
}
