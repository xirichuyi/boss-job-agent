...
export function ruleEvaluate(job, config) {
  ...
  const searchable = normalized(
    [job.title, job.company, job.location, job.summary, job.description].join(
      "\n",
    ),
  );
  const reasons = [];
  ...
  const excluded = criteria.excludeKeywords.find((term) =>
    searchable.includes(normalized(term)),
  );
  if (excluded)
    return {
      eligible: false,
      score: 0,
      reasons,
      risks: [`命中排除词：${excluded}`],
    };

  if (criteria.targetTitles.length) {
    const match = criteria.targetTitles.find((term) =>
      normalized(job.title).includes(normalized(term)),
    );
    if (match) {
      score += 20;
  ...
  const missingRequired = criteria.requiredKeywords.filter(
    (term) => !searchable.includes(normalized(term)),
  );
  ...
  if (criteria.cities.length) {
    const city = criteria.cities.find((item) =>
      normalized(job.location).includes(normalized(item)),
    );
    if (city) {
      score += 10;
