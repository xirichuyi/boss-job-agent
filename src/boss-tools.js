...
export class BossTools {
  ...
  async connect(targetId) {
    ...
    const target = targets.find(t => {
      try { return t.type === 'page' && (!targetId || t.id === targetId) && new URL(t.url).origin === 'https://www.zhipin.com'; }
      catch { return false; }
    });
    ...
    this.ws.onclose = () => {
      for (const p of this.pending.values()) { clearTimeout(p.timer); p.reject(new Error('浏览器连接已断开')); }
      this.pending.clear();
    };
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => { this.ws.close(); reject(new Error('CDP连接超时')); }, 5000);
      this.ws.onopen = () => { clearTimeout(timer); resolve(); };
      this.ws.onerror = () => { clearTimeout(timer); reject(new Error('CDP连接失败')); };
    });
  }
  call(method, params = {}) {
    if (this.ws?.readyState !== WebSocket.OPEN) return Promise.reject(new Error('CDP未连接'));
    return new Promise((resolve, reject) => {
      const id = ++this.sequence;
      const timer = setTimeout(() => { this.pending.delete(id); reject(new Error(`${method} 超时`)); }, 20000);
  ...
  async navigate(url) {
    ...
    if (expected.origin !== 'https://www.zhipin.com') throw new Error('只允许BOSS页面导航');
    let timeout;
    try { await this.call('Page.navigate', { url }); }
    catch (error) { if (!/超时/.test(error.message)) throw error; timeout = error; }
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      const r = await this.call('Runtime.evaluate', {
        expression: '({url:location.href,ready:document.readyState,hasBody:!!document.body})', returnByValue: true,
      }).catch(() => null);
      const page = r?.result?.value;
      if (page?.hasBody && ['interactive', 'complete'].includes(page.ready)) {
        const actual = new URL(page.url);
        if (actual.origin === expected.origin && actual.pathname === expected.pathname && actual.search === expected.search) return { ready: true, recoveredFromTimeout: Boolean(timeout) };
        if (/verify|passport/.test(actual.pathname)) throw new Error('导航进入登录或验证页，需要人工处理');
      }
  ...
  async read(path, params = {}) {
    const allowed = ['/wapi/zpgeek/resume/geek/preview/data.json', '/wapi/zpgeek/search/joblist.json'];
    ...
    if (r.exceptionDetails) throw new Error(r.exceptionDetails.exception?.description || '接口读取失败');
  ...
  async readResume() {
    ...
    const result = { name: data.baseInfo?.name, experience: data.baseInfo?.workYearDesc, sections: {} };
    for (const [key, value] of Object.entries(data)) {
      if (/expect|work|project|education|eduExp|skill|advantage|description|geekDesc|userDesc/i.test(key)) result.sections[key] = value;
    }
    return result;
  }
  async searchJobs({ query, page = 1 } = {}) {
    ...
    const jobs = data.jobList.map(j => ({
      ...
      companySize: j.brandScaleName, location: [j.cityName, j.areaDistrict, j.businessDistrict].filter(Boolean).join('·'),
      salary: j.salaryDesc, experience: j.jobExperience, degree: j.jobDegree,
      skills: j.skills, contacted: j.contact === true, valid: j.jobValidStatus === 1,
...
export const toolDefinitions = [
  { name: 'read_resume', description: '读取当前账号的职业简历信息', inputSchema: { type: 'object', properties: {}, additionalProperties: false } },
  { name: 'search_jobs', description: '搜索杭州岗位并剔除500人以下、未知规模及已联系岗位', inputSchema: { type: 'object', properties: { query: { type: 'string' }, page: { type: 'integer', minimum: 1, maximum: 30 } }, required: ['query'], additionalProperties: false } },
];
