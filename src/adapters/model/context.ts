...
export function profileContext(root) {
  ...
  if (
    manifest?.platformHash === digest(JSON.stringify(platform)) &&
    manifest?.localHash === digest(local) &&
    fs.existsSync(file)
  ) {
    const text = fs.readFileSync(file, "utf8");
    if (manifest.contextHash === digest(text))
      return {
        source: "platform_verified_digest",
...
export function jobContext(job) {
  return {
    ...
    salary: job.proof?.salary || job.salary,
    companySize: job.companySize || job.proof?.companySize,
    scaleEvidence: job.scaleEvidence,
    requirements: job.requirements,
    description: job.proof?.description || job.text || "",
  };
}
export function historyContext(history) {
  return {
    scope: history?.scope,
    empty: history?.empty,
    messages: (Array.isArray(history) ? history : history?.messages || []).map(
      (m) => ({ text: m.text, self: m.self, system: m.system, id: m.id }),
    ),
  };
}
