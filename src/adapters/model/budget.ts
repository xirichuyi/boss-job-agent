...
function budgets(root) {
  ...
  return (
    state.byModel ||
    (state.until || state.attempts
      ? { [state.model || "gpt-5.3-codex-spark"]: state }
      : {})
  );
}
function saveBudget(root, state) {
  atomicJson(root + "/memory/model-budget.json", {
    byModel: { ...budgets(root), [MODEL]: { ...state, model: MODEL } },
  });
}
...
export function budgetError(state) {
  const error = Object.assign(
    Error(MODEL + "用量限额冷却，等待至 " + state.until),
    { code: "MODEL_COOLDOWN", until: state.until },
  );
...
export function checkBudget(root) {
  ...
  if (state) throw budgetError(state);
}
export function recordUsage(root, logPath, now = Date.now()) {
  ...
  const limited = events.some(
    (e) =>
      ["error", "turn.failed"].includes(e.type) &&
      /usage limit|rate.limit|quota exceeded|用量.*限|额度.*不足/i.test(
  ...
  if (limited) {
    ...
    const until = new Date(
      Math.max(
        Date.parse(old.until || "") || 0,
        now + Math.min(120, 15 * 2 ** Math.min(attempts - 1, 3)) * 60000,
      ),
    ).toISOString();
    const state = {
      until,
      ...
      reason: MODEL + " usage limit",
      resetTimeSource: "local_backoff_not_provider_reset",
    };
    saveBudget(root, state);
    throw budgetError(state);
  }
  const completed = events.filter((e) => e.type === "turn.completed");
  if (completed.length && !cooldown(root, now))
    saveBudget(root, { until: null, attempts: 0 });
  if (completed.length)
    atomicJson(logPath + ".usage.json", {
      calls: completed.length,
      usage: completed.map((e) => e.usage),
    });
}
