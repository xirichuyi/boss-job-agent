...
import {
  serviceHealth,
  effectiveStatus,
} from "../../runtime/service-health.ts";
import { readState } from "../../storage/harness.ts";
import { callCodex, MODEL, REASONING_EFFORT } from "../model/codex.ts";
import { buildPrompt } from "../model/prompts.ts";
import { profileContext } from "../model/context.ts";
import { cooldown } from "../model/budget.ts";
...
export function snapshot(root) {
  ...
  const status = effectiveStatus(
    readJson(root + "/memory/scheduler-status.json", {}),
    service,
  );
  ...
  const contacts = readJson(root + "/memory/outreach-ledger.json", {
    contacts: [],
  }).contacts;
  return {
    ...
    inboxDiscovery: readJson(root + "/memory/inbox-discovery.json", null),
    ...
    reviews: (report.jobReviews || []).map((j) => ({
      company: j.company,
      title: j.title,
      rejected: j.rejected,
      decision: j.decision,
    ...
    contacts: contacts.map((e) => ({
      ...
      inboxRetry: e.inboxRetry,
      lastReplyDecision: e.lastReplyDecision,
      replyDraftPending: !!e.replyDraft,
      replyDeferredAt: e.replyDeferredAt,
      lastMessage: e.intent?.message,
      deliveryStage: e.deliveryStage,
      contactRecovery: e.contactRecovery,
      platformAttachment: e.platformAttachment,
      platformCheckedAt: e.platformCheckedAt,
    })),
    alerts: readJson(root + "/memory/alerts.json", [])
      .filter((a) => a.status === "open")
      .slice(-10)
      .map((a) => ({ kind: a.kind, reason: a.reason, at: a.lastSeen })),
  };
}
...
export function answerQuestion(root, question, history, runner = spawnSync) {
  ...
  if (paused)
    return `${MODEL}用量暂受限，下次检查：${paused.until}（UTC）。\n${statusText(context)}`;
  const key = randomUUID(),
    dir = root + "/memory/telegram-chat-runs";
  fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
  const output = `${dir}/${key}.txt`,
    log = fs.openSync(`${dir}/${key}.jsonl`, "wx", 0o600);
  const { text: prompt, version } = buildPrompt("telegram", {
    profile: profileContext(root),
    snapshot: context,
    history: history.slice(-6),
    earlierTurnsOmitted: Math.max(0, history.length - 6),
    question,
  });
  fs.writeFileSync(
    output + ".prompt-version.json",
    JSON.stringify({ kind: "telegram", version }),
    { mode: 0o600, flag: "wx" },
  );
  let run;
  try {
    ...
    if (e.code === "MODEL_COOLDOWN")
      return `${MODEL}用量暂受限，下次检查：${e.until}（UTC）。\n${statusText(context)}`;
    throw e;
  } finally {
    fs.closeSync(log);
  }
...
export function ingest(state, updates, chatId) {
  ...
  for (const u of updates) {
    if (!Number.isSafeInteger(u.update_id) || u.update_id < (state.offset || 0))
      continue;
    if (
      authorizedMessage(u, chatId, state.startedAt) &&
      !state.queue.some((q) => q.id === u.update_id)
    )
      state.queue.push({
        id: u.update_id,
