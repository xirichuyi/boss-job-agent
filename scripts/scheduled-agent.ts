...
import { recoveryPlan, retryDelay } from "../src/domain/recovery.ts";
import { raiseAlert } from "../src/storage/alerts.ts";
...
if (!lease) {
  console.log("executor_already_running");
  process.exit(0);
}
process.env.JOB_AGENT_LEASE = lease;
process.on("exit", () => store.release(lease));
const config = {
  ...readState(root + "/memory/schedule.json"),
  ...AGENT.schedule,
};
...
if (
  ...
  config.perRun < 0 ||
  config.perRun > 10 ||
  !Number.isInteger(config.dailyNewContactLimit) ||
  config.dailyNewContactLimit < 1 ||
  config.dailyNewContactLimit > 70
)
  throw new Error("Invalid contact limits");
// Installing the timer does not imply that the send/receipt adapters are ready.
if (!config.automationReady) {
  status("paused", { reason: config.phase });
  process.exit(0);
}
if (!/^[0-9a-f-]{36}$/.test(config.threadId))
  throw new Error("Invalid dedicated thread id");
...
if (readState(outstanding)) {
  ...
  const due = ["completed", "skipped", "quarantined"].includes(previous.status)
    ? Date.parse(previous.at) + config.intervalMinutes * 60000
    : Date.parse(retry.nextAt || "") || 0;
  if (!queued && due > Date.now()) {
    status("waiting", {
      cycle: previous.id,
      nextAt: new Date(due).toISOString(),
  ...
  if (!["completed", "skipped", "quarantined"].includes(previous.status)) {
    ...
    if (plan.action === "authentication") {
      ...
      if (health.status === 0)
        plan = recoveryPlan(previous, report, ledger, true);
    }
    if (plan.action === "review" || plan.action === "authentication") {
      ...
      retry.nextAt = new Date(Date.now() + 5 * 60000).toISOString();
    ...
    if (plan.action === "quarantine") {
      for (const entry of ledger.contacts) {
        if (
          (entry.cycle === previous.id && entry.status === "outcome_unknown") ||
          plan.unresolved.includes(entry.job.id)
        ) {
          entry.status = "quarantined";
          entry.quarantineReason = plan.reason;
          raiseAlert(root, {
            kind: "unknown_send",
            cycle: previous.id,
            contact: entry.job.id,
            reason:
              "某联系人的发送结果不明，已隔离，禁止自动重发。其他岗位继续。",
          });
        }
      }
...
if (!Number.isInteger(quota.reserved) || quota.reserved < 0)
...
const allocation = budget
  ...
  : Math.max(
      0,
      Math.min(config.perRun, config.dailyNewContactLimit - quota.reserved),
    );
...
atomicBatch(
  [
    ...
    [outstanding, cycle],
    [
      `${root}/memory/cycles/${cycle.id}.json`,
      {
        id: cycle.id,
        startedAt: cycle.at,
        status: "queued",
...
if (
  finished.id !== cycle.id ||
  !["completed", "blocked"].includes(finished.status) ||
  !Number.isInteger(finished.result?.newContacts) ||
  finished.result.newContacts < 0 ||
  finished.result.newContacts > allocation
) {
  finished = {
    ...cycle,
    status: "blocked",
    reason: "worker_failed_or_result_unconfirmed",
...
if (finished.status === "completed") {
  ...
  const contactOnly =
    (finished.reason || "").includes("CONTACT_RECOVERY_PENDING") ||
    (finished.reason || "").includes("联系人暂未找到");
  retry.nextAt = new Date(
    Date.now() +
      (contactOnly
  ...
  atomicJson(retryPath, retry);
  raiseAlert(root, {
    kind: /登录|验证|访问受限/.test(finished.reason || "")
      ? "authentication"
      : "cycle_failed",
    cycle: cycle.id,
    reason: finished.reason || "任务失败，将自动核对并恢复",
    nextAt: retry.nextAt,
  });
}
